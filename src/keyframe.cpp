...
// 将坐标去畸变，并投影到归一化平面
void liftProjective(cv::Mat &K, cv::Mat &distCoef, 
					vector<cv::KeyPoint> &keypoints, vector<cv::KeyPoint> &keypoints_norm)
{
...
KeyFrame::KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, Vector3d &_T_w_i, Matrix3d &_R_w_i,
					...
					vector<cv::Point3f> &_point_3d, vector<cv::Point2f> &_point_2d_uv, vector<cv::Point2f> &_point_2d_norm,
					vector<double> &_point_id, vector<cv::KeyPoint> &_window_keypoints, vector<BRIEF::bitset> &_window_brief_descriptors)
{
	time_stamp = _time_stamp;
	index = _index;
	//vio_T_w_i = _vio_T_w_i;
	...
	point_2d_norm = _point_2d_norm;
	point_id = _point_id;
	window_keypoints = _window_keypoints;
...
void KeyFrame::computeBRIEFPoint()
{
	...
	*/
	liftProjective(K_camera, distCoef, keypoints, keypoints_norm);
}
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    PnP_R_old = R_w_c_old * qic.transpose();		// 相当于query在地图下的位姿
    PnP_T_old = T_w_c_old - PnP_R_old * tic;		

}


bool KeyFrame::findConnection(KeyFrame* old_kf, Eigen::Vector3d &old_kf_T, Eigen::Matrix3d &old_kf_R)
{
	TicToc tmp_t;
	...
	// 调试打开
		...
		if (DEBUG_IMAGE)
	    {
	        ...
	        path <<  "/home/yabao/output/loop_image/"
	                << index << "-"
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		...
		// 得到query的全局位姿(保存的地图里，vins的坐标系和lio的坐标系绕z轴旋转了180度)
		Eigen::AngleAxisd rotation_vector(M_PI, Vector3d(0, 0, 1));
		Eigen::Matrix3d vins_to_lio = rotation_vector.toRotationMatrix();
		old_kf_T = vins_to_lio * PnP_T_old;
		old_kf_R = vins_to_lio * PnP_R_old;
		//std::cout << "T is: " << PnP_T_old.transpose() << std::endl;
		//std::cout << "R is: " << std::endl << PnP_R_old << std::endl;
	    reduceVector(matched_2d_cur, status);
	    reduceVector(matched_2d_old, status);
	    reduceVector(matched_2d_cur_norm, status);
	    reduceVector(matched_2d_old_norm, status);
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            putText(notation, "previous frame: " + to_string(index) + "  sequence: " + to_string(sequence), cv::Point2f(20, 30), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);

	            putText(notation, "current frame: " + to_string(old_kf->index) + "  sequence: " + to_string(old_kf->sequence), cv::Point2f(20 + COL + gap, 30), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);
	            cv::vconcat(notation, loop_match_img, loop_match_img);

				cv::imshow("pnp_match", loop_match_img);
				cv::waitKey();
...
{
	ar & time_stamp & index & local_index;
	ar & vio_T_w_i & vio_R_w_i & T_w_i & R_w_i & origin_vio_T & origin_vio_R; 
	ar & image & thumbnail;
	ar & point_3d & point_2d_uv & point_2d_norm;
	ar & point_id;
	ar & keypoints & keypoints_norm & window_keypoints;
	ar & brief_descriptors & window_brief_descriptors;
	ar & has_fast_point & sequence & has_loop & loop_index;
	ar & loop_info;
}
