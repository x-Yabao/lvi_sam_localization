...
public: 
    ...
    void removeLastScancontextAndKeys();    //yabao
...
public:
    ...
    const int    NUM_EXCLUDE_RECENT = 1;         // yabao, exclude itself
    ...
    // const double SC_DIST_THRES = 0.3;       // original test
    const double SC_DIST_THRES = 0.5;       // yabao
    ...
    std::vector<Eigen::MatrixXd> polarcontexts_;            // scan context
    std::vector<Eigen::MatrixXd> polarcontext_invkeys_;     // ring key
    std::vector<Eigen::MatrixXd> polarcontext_vkeys_;       // sector key

    KeyMat polarcontext_invkeys_mat_;                       // ring key
